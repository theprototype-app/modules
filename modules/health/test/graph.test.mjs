...
import { indexGraph, targetsOf, chainsOf, feedersOf, triggerSourcesOf, zoneOf, recipe, nameOf } from '../src/graph.js';
...
export function run(check) {
	...
	const nodes = [
		...
		n('zdmg', 'damage', { source: 'zone' }),
		n('zsel', 'objectselector', { selected: 'lava-uuid' })
	];
	const edges = [
		e('dmg', 'cnt', 'pulse'),
		e('cnt', 'hp', 'damage'),
	...
	check(targetsOf(g.byId.get('hp'), g).join() === 'crate-uuid', 'a health targets the object wired into `target`');
	...
	check(zoneOf(g.byId.get('zdmg'), g) === 'lava-uuid', 'a zone damage node reads the object wired into `zone`');
	check(zoneOf(g.byId.get('dmg'), g) === null, 'COUNTERFACTUAL: a node with no zone wire has no zone');
	...
	check(r.nodes[2].data.scope === 'object' && r.nodes[2].data.name === 'enemy' && !('whilePlaying' in r.nodes[2].data), 'the health carries the form (and is no effect: no whilePlaying)');
	...
	check(JSON.stringify(r.edges) === JSON.stringify([{ from: 0, to: 1, handle: 'pulse' }, { from: 1, to: 2, handle: 'damage' }, { from: 3, to: 1, handle: 'reset' }, { from: 4, to: 2, handle: 'target' }]), 'the wires: pulse, damage, reset, selector -> health.target');
